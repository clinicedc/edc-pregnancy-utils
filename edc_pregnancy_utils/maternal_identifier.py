...
class MaternalIdentifier(SubjectIdentifier):
    ...
    def __init__(self, last_name=None, **kwargs):
        ...
        infant_identifiers = []
        for obj in IdentifierModel.objects.filter(linked_identifier=self.identifier).order_by(
            "linked_identifier"
        ):
    ...
    def deliver(
        self, live_infants, model, birth_orders=None, create_registration=None, **kwargs
    ):
        ...
        if self.infants:
            raise MaternalIdentifierError(
                f"Infant identifiers already created for this mother. " f"Got {self.infants}"
            )
        else:
